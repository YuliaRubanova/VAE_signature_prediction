...
def mean_squared_loss(labels, predicted):
	return np.sum(np.square(labels - predicted), axis=-1)
...
def two_step_vae_plus_nn_predict(rec_params, gen_params, data, latent_dim):
	...
	z_first_tp_means = z_latents[:,:latent_dim]
	z_first_tp_stds = z_latents[:,latent_dim:]
...
if __name__ == '__main__':
	...
	# ==============================================================
	# Train second step: NN for recognition net on the first time step
	gen_layer_optimized, rec_layer_optimized = optimized_params
	z_means, z_stds = nn_predict_gaussian(rec_layer_optimized, train_samples)
	...
	def objective_first_tp_nn(rec_params_first_tp, iter):
		data_idx = batch_indices(iter)

		predicted_z_labels = neural_net_predict(rec_params_first_tp, train_first_tp[data_idx])
		loss =  mean_squared_loss(z_labels[data_idx], predicted_z_labels)
		
		return np.mean(loss)

	# Get gradients of objective using autograd.
	objective_first_tp_grad = grad(objective_first_tp_nn)
	...
	def print_perf_first_tp_net(rec_params_first_tp, iter, grad):
		if iter % num_batches == 0:
			# plt.cla()
			loss = objective_first_tp_nn(rec_params_first_tp, iter)
			print("{:15}|{:20} ".format(iter//num_batches, loss))

			with open(log_file, "a") as log:
				log.write("{:15}|{:20} ".format(iter//num_batches, loss))
